...
async def chapters_for_user(m):
    ...
    text="[5938069973535559743] <b>از منوی زیر فصل موردنظر خود را انتخاب کنید:</b>"
...
async def l_list(m:Message):
    if not await is_admin(m.from_user.id): return
    x=await db(); c=await x.execute("SELECT l.*,c.title ct FROM lessons l LEFT JOIN chapters c ON c.id=l.chapter_id ORDER BY COALESCE(c.position,999999),l.position,l.id"); r=await c.fetchall(); await x.close()
    text="\n".join(f"#{z['id']} | {'بدون فصل' if not z['chapter_id'] else z['ct']} | {html.escape(z['title'])} | {z['duration']}s" for z in r)
    await m.answer("🎬 <b>درس‌ها</b>\n\n"+(text or "هیچ درسی وجود ندارد."),parse_mode="HTML",reply_markup=lesson_manage_kb())

@dp.message(F.text=="🗑 حذف درس")
async def l_del(m:Message,state:FSMContext):
    if await is_admin(m.from_user.id): await state.set_state(S.del_l_id); await m.answer("🗑 شناسه درس را برای حذف وارد کنید:")
@dp.message(S.del_l_id)
async def l_del_id(m:Message,state:FSMContext):
    try: lid=int(m.text)
    except: return await m.answer("❗ شناسه عددی وارد کنید.")
    x=await db(); await x.execute("DELETE FROM lessons WHERE id=?",(lid,)); await x.commit(); await x.close(); await state.clear(); await m.answer("🗑 درس حذف شد.",reply_markup=lesson_manage_kb())

@dp.message(F.text=="✏️ ویرایش درس")
async def l_edit(m:Message,state:FSMContext):
    if await is_admin(m.from_user.id): await state.set_state(S.edit_l_id); await m.answer("✏️ شناسه درس را وارد کنید:")
@dp.message(S.edit_l_id)
async def l_edit_id(m:Message,state:FSMContext):
    try: lid=int(m.text)
    except: return await m.answer("❗ شناسه عددی وارد کنید.")
    await state.update_data(id=lid); await state.set_state(S.edit_l_title); await m.answer("عنوان جدید را وارد کنید:")
@dp.message(S.edit_l_title)
async def l_edit_title(m:Message,state:FSMContext):
    await state.update_data(title=m.text); await state.set_state(S.edit_l_desc); await m.answer("توضیحات جدید را وارد کنید یا «➡️ بعدی» را بزنید:",reply_markup=ReplyKeyboardMarkup(keyboard=[[kb_button("➡️ بعدی","success")],[kb_button("⬅️ بازگشت","danger")]],resize_keyboard=True,is_persistent=True))
@dp.message(S.edit_l_desc)
async def l_edit_desc(m:Message,state:FSMContext):
    z=await state.get_data(); desc="" if m.text=="➡️ بعدی" else (m.text or ""); x=await db(); await x.execute("UPDATE lessons SET title=?,description=? WHERE id=?",(z["title"],desc,z["id"])); await x.commit(); await x.close(); await state.clear(); await m.answer("✅ درس ویرایش شد.",reply_markup=lesson_manage_kb())
