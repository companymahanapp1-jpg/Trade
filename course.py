...
async def can_open(uid,lid):
    ...
    c=await x.execute("SELECT id FROM lessons WHERE chapter_id=? AND position<? ORDER BY position DESC,id DESC LIMIT 1",(cur["chapter_id"],cur["position"]));p=await c.fetchone()
    if not p:await x.close();return True
    c=await x.execute("SELECT quiz_passed FROM progress p JOIN users u ON u.id=p.user_id WHERE u.telegram_id=? AND p.lesson_id=?",(uid,p["id"]));r=await c.fetchone();await x.close();return bool(r and r["quiz_passed"])
